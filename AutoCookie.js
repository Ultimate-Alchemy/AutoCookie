...
AC.Auto.prototype.run = function(runImmediately, interval) {
	...
	if (typeof interval === 'number' && interval > 0) {
		...
		if (setting && setting.units === 'cps') {
			...
			var self = this;
			this.intvlID = setInterval(function() {
				for (var i = 0; i < callsPerTick; i++) self.actionFunction();
			}, 1);
		} else {
...
AC.Display.addSetting = function(auto, setting) {
	...
	if (setting.type === 'deprecated' || setting.type === 'header') {
		...
		// Add a slider + text input for this setting's values.
		var div = document.createElement('div');
		...
		// Right side: editable text input + units label
		var rightDiv = document.createElement('div');
		rightDiv.style.cssFloat = 'right';

		var textInput = document.createElement('input');
		textInput.type = 'number';
		textInput.min = setting.min;
		textInput.step = setting.step;
		textInput.value = auto[setting.name];
		textInput.id = auto.name + ' ' + setting.name + ' TextInput';
		...
		unitsSpan.textContent = ' ' + setting.units;
		unitsSpan.style.fontSize = '12px';
		rightDiv.appendChild(unitsSpan);

		// Status indicator (ON/OFF) for Interval settings
		if (setting.name === 'Interval') {
			var statusSpan = document.createElement('span');
			statusSpan.id = auto.name + ' ' + setting.name + ' Status';
			statusSpan.style.marginLeft = '8px';
			statusSpan.style.fontSize = '11px';
			statusSpan.style.fontWeight = 'bold';
			if (auto[setting.name] > 0) {
		...
		slider.value = Math.min(auto[setting.name], setting.max);
		slider.id = auto.name + ' ' + setting.name + ' Slider';

		// Sync: slider -> text input + value
		slider.oninput = function() {
			var val = parseFloat(l(auto.name + ' ' + setting.name + ' Slider').value);
			auto[setting.name] = val;
			l(auto.name + ' ' + setting.name + ' TextInput').value = val;
			if (setting.name === 'Interval') {
				var st = l(auto.name + ' ' + setting.name + ' Status');
				if (st) { st.textContent = val > 0 ? 'ON' : 'OFF'; st.style.color = val > 0 ? '#0f0' : '#f00'; }
			}
		}
		if (setting.name === 'Interval') {
		...
		// Sync: text input -> slider + value
		textInput.oninput = function() {
			var val = parseFloat(this.value);
			if (isNaN(val) || val < setting.min) val = setting.min;
			auto[setting.name] = val;
			// Clamp the slider to its max, but the actual value can exceed it
			l(auto.name + ' ' + setting.name + ' Slider').value = Math.min(val, setting.max);
			if (setting.name === 'Interval') {
				var st = l(auto.name + ' ' + setting.name + ' Status');
				if (st) { st.textContent = val > 0 ? 'ON' : 'OFF'; st.style.color = val > 0 ? '#0f0' : '#f00'; }
			}
